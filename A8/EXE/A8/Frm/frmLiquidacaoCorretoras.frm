...
'Liquidação de operações efetuadas em corretoras
        ...
        For Each objDomNode In xmlRetLeitura.selectNodes("Repeat_DetalheMensagem/*")
            ...
            Else
                With lvwMensagem.ListItems(strListItemKey)
                    ...
                    'verifica se já existe uma Mensagem SPB conciliada à esta operação, se Sim, então acrescenta uma linha nova para a MensagemSPB atual
                    'If .Tag = "" Then 'não havia Mensagem conciliada
                    ...
                    'Else 'já há outra Mensagem conciliada
...
On Error GoTo ErrorHandler
    ...
    For Each objListItem In objListView.ListItems
        ...
        'para comparação da key da operação e da mensagem, considerar apenas os 7 primeiros campos das keys (se mensagemSPB for STR008R2 ou PAG0108R2), ou se
        'visto que do 8º campo em diante (CD_LOTE, SG_SIST, DT_OPER_ATIV, NU_CTRL_IF) não fazem parte da chave de conciliação da operação com mensagem R2
        ...
        If strKeyOperacao = strKeyMensagemSemNumCtrlIF Then
            ...
            If Split(strKeyOperacaoOriginal, "|")(KEY_MSG_NU_CTRL_IF) = "" Then 'garante que não há outra MensagemSPB conciliada a este lote
...
On Error GoTo ErrorHandler
    ...
    With Me.lvwMensagem.ColumnHeaders
        ...
        If PerfilAcesso = BackOffice Then
            ...
            .Add , , "Quantidade Operações Lote", 3000
            'RATS 1211 - Inclusão de Campos na Conciliação
        ...
        Else
            ...
            Else
                ...
                .Add , , "Valor Operação Apurado SLCC", 2600, lvwColumnRight
        ...
        If objListItem.Checked Then
            ...
            If ((dblValorTotalLote - dblValorNet) <> 0) Or (dblValorTotalLote = 0) Then
                If dblValorTotalLote = 0 Then
                    MsgBox "Valor do Lote não apurado pelo SLCC. Não é possível efetuar a Liberação.", vbExclamation + vbOKOnly, Me.Caption
                    Exit Function
                ElseIf intAcaoProcessamento = enumAcaoConciliacao.AdmAreaLiberarContingencia Then
                    ...
                    Else
                        If cboTipoJustificativa.Text = Empty Or txtComentario.Text = Empty Then
                            MsgBox "É necessário informar a Justificativa e o Comentário.", vbExclamation + vbOKOnly, Me.Caption
                            Exit Function
                        Else
                ...
                ElseIf PerfilAcesso = enumPerfilAcesso.AdmArea Then
                    MsgBox "Valor do Lote diferente do Valor apurado pelo SLCC. O lote só poderá ser liberado pela Contingência.", vbExclamation + vbOKOnly, Me.Caption
                    Exit Function
                End If
            End If
            ...
            If intTotalOperLote <> intTotalOper Then
                ...
                If intAcaoProcessamento = enumAcaoConciliacao.AdmAreaLiberarContingencia Then
                    If MsgBox("Quantidade de Operações do Lote diferente da quantidade apurada pelo SLCC. Deseja liberar o lote assim mesmo?", vbQuestion + vbYesNo, Me.Caption) = vbNo Then
                        Exit Function
                    Else
                    ...
                    End If
                ElseIf PerfilAcesso = enumPerfilAcesso.BackOffice Then
                    If MsgBox("Quantidade de Operações do Lote diferente da quantidade de Operações apuradas pelo SLCC. Deseja liberar o lote pelo valor apurado pelo SLCC?", vbExclamation + vbYesNo, Me.Caption) = vbNo Then
                        Exit Function
                    End If
                ElseIf PerfilAcesso = enumPerfilAcesso.AdmArea Then
                    MsgBox "Quantidade de Operações do Lote diferente da quantidade de Operações apuradas pelo SLCC. O lote só poderá ser liberado por Contingência", vbExclamation + vbOKOnly, Me.Caption
                    Exit Function
                End If
            End If
        End If
                ...
                Else
                    If intAcaoProcessamento = enumAcaoConciliacao.AdmAreaLiberar Then
                        ...
                        If Trim$(objListItem.SubItems(COL_MSG_VALR_OPER)) = vbNullString And _
                           objListItem.SubItems(COL_MSG_CNPT) = "Externa" Then
                            ...
                            flValidarItensProcessamento = "Valor do Lote não apurado pelo SLCC. Não é possível efetuar a Liberação."
                            Exit Function
                            
                        ElseIf objListItem.ListSubItems(COL_MSG_DIFE).ForeColor = vbRed And _
                               objListItem.SubItems(COL_MSG_CNPT) = "Externa" Then
                            flValidarItensProcessamento = "Liberação não permitida: um ou mais item não batido. Se necessário, utilize o processo Liberar em Contingência"
                            Exit Function
                    ...
                    ElseIf intAcaoProcessamento = enumAcaoConciliacao.AdmAreaLiberarContingencia Then
                        ...
                        If Trim$(objListItem.SubItems(COL_MSG_VALR_OPER)) = vbNullString Then
                            ...
                            flValidarItensProcessamento = "Valor do Lote não apurado pelo SLCC. Não é possível efetuar a Liberação por Contingência."
                            Exit Function
                        End If
...
'Calcula a quantidade de operações do lote
...
Property Let PerfilAcesso(pPerfil As enumPerfilAcesso)
    ...
    Select Case pPerfil
        Case enumPerfilAcesso.BackOffice
            Me.Caption = "Liberação - Liquidação com Corretoras (Backoffice)"
        Case enumPerfilAcesso.AdmArea
            Me.Caption = "Liberação - Liquidação com Corretoras (Administrador de Área)"
    End Select
