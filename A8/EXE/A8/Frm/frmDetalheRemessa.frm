...
'' Objeto responsável pela consulta dos detalhes sobre uma remessa rejeitada,
...
'Define se o form já foi descarregado, para tratar o doevents nos eventos do explorer
Public blnUnload                            As Boolean

Private Const MOTIVO_DA_REJEICAO            As Integer = 0
Private Const REMESSA_REJEITADA             As Integer = 1

'' Retorna a cor para cada um dos níveis do HTML
...
'' Cria o HTML com as informações sobre a remessa.
...
'' Interage com a camada de controle de caso de uso, método:  A8MIU.
'' clsRemessaRejeitada.ObterXmlErroRemessaRejeitada
...
'' Exibe os detalhes a respeito do motivo de rejeição da remessa
...
ErrorHandler:
    ...
    mdiLQS.uctlogErros.MostrarErros Err, "wbMotivo_DocumentComplete", Me.Caption
...
ErrorHandler:
    ...
    mdiLQS.uctlogErros.MostrarErros Err, "wbRemessa_DocumentComplete", Me.Caption
End Sub
